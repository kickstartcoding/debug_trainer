...
var $author$project$Model$SavedData$FileData = F5(
	function (originalContent, updatedContent, lineNumber, breakType, changeDescription) {
		return {breakType: breakType, changeDescription: changeDescription, lineNumber: lineNumber, originalContent: originalContent, updatedContent: updatedContent};
	});
...
var $author$project$Commands$Break$Cmd$init = F3(
				...
				dataPresentCmdFunc: F2(
					function (_v0, _v1) {
						return $author$project$Ports$printAndExitSuccess(
							'\n\n' + ($author$project$Utils$Types$FilePath$toString(filepath) + (' has already had a change introduced to it. ' + ('If you want to reset it to try breaking it in a different way, ' + (' run:\n\ndebug_trainer reset ' + ($author$project$Utils$Types$FilePath$toString(filepath) + '\n\nAfter that you\'ll be able to run the `break` command again.'))))));
					}),
				filepath: filepath,
				model: model
			});
		...
		return $author$project$Utils$Cmd$fromFileData(
			{
				...
				dataPresentCmdFunc: F2(
					function (savedData, _v0) {
						...
						return $author$project$Ports$printAndExitSuccess(
							changeDescription + (' on line ' + ($elm$core$String$fromInt(lineNumber) + ' of the original file')));
					}),
				filepath: filepath,
				model: model
			});
...
var $author$project$Model$Break = F2(
	function (a, b) {
...
var $author$project$Ports$exitSuccess = _Platform_outgoingPort(
	'exitSuccess',
	function ($) {
		return $elm$json$Json$Encode$null;
	});
var $author$project$Utils$Cmd$exitIfAllSavesAreComplete = function (fileSaveStatus) {
	return $author$project$Model$allSavesComplete(fileSaveStatus) ? $author$project$Ports$exitSuccess(_Utils_Tuple0) : $elm$core$Platform$Cmd$none;
};
var $author$project$Commands$Break$Update$confirmSaveDataFileWrite = F3(
	function (targetFilepath, fileSaveStatus, model) {
		...
		var newModel = _Utils_update(
			model,
			{
				command: A2($author$project$Model$Break, targetFilepath, newFileSaveStatus)
			});
		return _Utils_Tuple2(
			newModel,
			$author$project$Utils$Cmd$exitIfAllSavesAreComplete(newFileSaveStatus));
	});
var $author$project$Commands$Break$Update$confirmTargetFileWrite = F3(
	function (targetFilepath, fileSaveStatus, model) {
		var newFileSaveStatus = _Utils_update(
			fileSaveStatus,
...
var $author$project$Breakers$CaseSwap$run = function (_v0) {
	...
	return A2(
		...
		function (_v1) {
			...
			var newContent = $author$project$Breakers$Utils$segmentsToContent(
				A3(
			...
			return {breakType: $author$project$Utils$Types$BreakType$CaseSwap, changeDescription: 'changed `' + (content + ('` to `' + (newWord + '`'))), lineNumber: lineNumber, originalContent: originalFileContent, updatedContent: newContent};
		},
		...
		function (_v1) {
			...
			var newContent = $author$project$Breakers$Utils$segmentsToContent(
				A3(
					$elm_community$list_extra$List$Extra$setAt,
					index,
					A3($author$project$Parsers$Generic$Segment$Segment, offset, withoutBracket, $author$project$Parsers$Generic$Segment$ReturnStatement),
					segments));
			var lineNumber = A2(
				$author$project$Utils$FileContent$rowFromOffset,
				function () {
	...
	return A2(
		...
		function (_v1) {
			...
			var lineNumber = A2(
				...
				originalFileContent);
			return {breakType: $author$project$Utils$Types$BreakType$RemoveReturn, changeDescription: 'removed a `return`', lineNumber: lineNumber, originalContent: originalFileContent, updatedContent: newContent};
		},
...
var $author$project$Commands$Break$Update$BreakFile$run = F4(
	function (filepath, fileSaveStatus, fileContent, model) {
		var maybeChange = function () {
			var _v1 = $author$project$Parsers$Generic$Parser$run(fileContent);
			if (_v1.$ === 'Ok') {
				var segments = _v1.a;
				var maybeBreakType = A2($author$project$Commands$Break$Update$BreakFile$chooseBreakType, segments, model.randomNumbers);
				var breakRunnerData = {originalFileContent: fileContent, randomNumber: model.randomNumbers.segmentToBreakInt, segments: segments};
		...
		if (maybeChange.$ === 'Just') {
			...
			var newSavedData = A2(
				...
				{fileData: newFileData, filepath: filepath, workingDirectory: model.workingDirectory},
				oldSavedData);
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						command: A2($author$project$Model$Break, filepath, fileSaveStatus)
					}),
...
var $author$project$Commands$Break$Update$update = F4(
	function (targetFilepath, fileSaveStatus, action, model) {
		if (action.$ === 'SuccessfulFileRead') {
			var content = action.a.content;
			return A4($author$project$Commands$Break$Update$BreakFile$run, targetFilepath, fileSaveStatus, content, model);
		} else {
			var path = action.a.path;
			return _Utils_eq(
				$author$project$Utils$Types$FilePath$fromString(path),
				targetFilepath) ? A3($author$project$Commands$Break$Update$confirmTargetFileWrite, targetFilepath, fileSaveStatus, model) : A3($author$project$Commands$Break$Update$confirmSaveDataFileWrite, targetFilepath, fileSaveStatus, model);
		}
	});
			...
			switch (_v1.a.$) {
				case 'Break':
					if (_v1.b.$ === 'BreakAction') {
						var _v2 = _v1.a;
						var filepath = _v2.a;
						var fileSaveStatus = _v2.b;
						var subAction = _v1.b.a;
						var _v3 = A4($author$project$Commands$Break$Update$update, filepath, fileSaveStatus, subAction, model);
						var newModel = _v3.a;
						var cmd = _v3.b;
						return _Utils_Tuple2(
							newModel,
...
exports.default = default_1;
},{"../logging":"logging.ts"}],"ports/writeFile.ts":[function(require,module,exports) {
"use strict";
...
function default_1(program) {
  program.ports.writeFile.subscribe(function (fileData) {
    ...
    logging_1.devLog("Writing new content of " + fileData.path + "...");
...
},{"../logging":"logging.ts"}],"ports/exitSuccess.ts":[function(require,module,exports) {
"use strict";
...
function default_1(program) {
  program.ports.exitSuccess.subscribe(function () {
    return process.exit(0);
  });
}

exports.default = default_1;
},{}],"../tests_end_to_end/testHelpers.ts":[function(require,module,exports) {
"use strict";
...
exports.clearTestFile = exports.clearSaveFile = exports.readTestFile = exports.createTestFileWithContent = exports.runResetCommand = exports.runLineExplainCommand = exports.runLineNumberHintCommand = exports.runErrorTypeHintCommand = exports.runBreakCommand = exports.dataFileName = exports.testFileName = void 0;
...
function runLineExplainCommand() {
  return runCommand("explain " + exports.testFileName);
}

exports.runLineExplainCommand = runLineExplainCommand;
...
exports.load = load;
},{"./logging":"logging.ts","../tests_end_to_end/testHelpers":"../tests_end_to_end/testHelpers.ts"}],"main.ts":[function(require,module,exports) {
"use strict";
...
var exitSuccess_1 = __importDefault(require("./ports/exitSuccess"));
...
function run() {
  ...
  var program = Main_elm_1.Elm.Main.init({
    flags: {
      ...
      data: data,
      versionMessage: "5.0.0"
    }
  });
  var portFunctions = [print_1.default, printAndExitFailure_1.default, printAndExitSuccess_1.default, exitSuccess_1.default, readFile_1.default, writeFile_1.default];
...
},{"../elm/Main.elm":"../elm/Main.elm","./ports/print":"ports/print.ts","./ports/printAndExitFailure":"ports/printAndExitFailure.ts","./ports/printAndExitSuccess":"ports/printAndExitSuccess.ts","./ports/readFile":"ports/readFile.ts","./ports/writeFile":"ports/writeFile.ts","./ports/exitSuccess":"ports/exitSuccess.ts","./savedData":"savedData.ts","./logging":"logging.ts"}]},{},["main.ts"], null)
